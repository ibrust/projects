...
#It looks like this file returns either a refreshed access token for the authorization flow, or a token for client credentials flow, 
#depending on which flow is used
...
def get_auth_key(client_id, client_secret):
    ...
    # this is not b64decode. why did we need to do this? we wanted the string representation so that it can be sent with the request payload, apparently.
    return encoded_key.decode('utf-8')


# placing import down here avoids partially imported module error due to import cycle
...
def _client_credentials(conf):
    auth_key = get_auth_key(conf.client_id, conf.client_secret)
    headers = {'Authorization': f'Basic {auth_key}'}
    options = {'grant_type': 'client_credentials', 'json': True}
    response = requests.post('https://accounts.spotify.com/api/token', headers=headers, data=options)
    content = json.loads(response.content.decode('utf-8'))
