...
#This retrieves the initial authorization token. You click on the link at http://localhost:3000/ to make a request for the token. 
#Spotify will send a response and call the redirect uri which is http://localhost:3000/callback. Flask will execute a function that 
#stores this token in a file called .pytify when this uri is invoked.
...
def _authorization_code_request(auth_code):
    ...
    response = requests.post(config.access_token_url, headers=headers, data=options)
